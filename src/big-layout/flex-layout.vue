        ...
        >
          <div class="table-content">
            <el-table :data="tableData">
              <el-table-column property="source" label="数据库" />
              <el-table-column property="tb_name" label="数据表" />
              <el-table-column property="remark" label="数据备注" />
              <el-table-column property="posted" label="同步进度" />
              <el-table-column
                property="synced"
                :formatter="
                  v => {
                ...
                label="同步状态"
              />
              <el-table-column property="finish_at" label="最近同步时间" />
              <el-table-column property="sync_type" label="同步类型" />
            </el-table>
            <el-pagination
              background
              :current-page="page"
...
<script>
import dataSource from '@/assets/json/dataSource.json'
const { tables, datasource } = dataSource
let tableData = tables.map(v => {
  const source = datasource.find(j => {
    return v.db_id === j.db_id
  })
  if (source !== -1) {
    v.source = source.ds_name
  }
...
tableData = tableData
  // .filter(v => {
  //   return v.source !== "平台系统表" && v.source !== "字典信息";
  // })
  .map(v => {
    v.source = v.source + '库'
    return v
...
export default {
  ...
  created() {
    ...
    //   if (target.clientWidth > 5000) width = 6720
...
.table-container {
  ...
  .popper__arrow,
  .popper__arrow::after {
    border-bottom-color: #171833 !important;
  }
  /** table 样式 */
  .el-table {
...
.page-size-popper {
  ...
  .popper__arrow,
  .popper__arrow::after {
    border-bottom-color: #212342 !important;
  }
