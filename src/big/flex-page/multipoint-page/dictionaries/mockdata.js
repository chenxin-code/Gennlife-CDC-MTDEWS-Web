...
const people1 = {
  ...
  personWarnEventList: [{ eventRule: '重点人群' }, { eventRule: '核酸检测' }]
}
// 接驳司机
const people2 = {
  name: '王*阳',
...
const people3 = {
  ...
  area: '清城区',
  address: '广东省清远市清城区石角镇美林湖怡景湾6-2梯1506',
  workplace: '广东省清远市清城区石角镇新民小学',
  crowdCode: '6',
  testDate: '2020-12-08',
  testResult: '初筛阳性',
  school: '广东省清远市清城区石角镇新民小学',
  className: '一年级2班',
  hospital: '清远市第一人民医院',
  symptom: '咳嗽，发热到医院诊',
...
const areaList = [
  ...
  },
  {
    ...
    addressList: [
      {
        longitude: 113.074496,
        latitude: 23.518981,
        placeName: '广东省清远市清城区石角镇新民小学',
        address: '清远市清城区',
        organizationName: '已推送 清远市清城区疾病预防控制中心',
        message: '已推送至流调系统',
        cardId: '440981201403295420',
...
export function step11() {
  // 展示列表、 及个人信息
  people1.testDate = '2020-12-20'
  people1.testResult = '阳性'

  people3.testDate = '2020-12-22'
  people3.testResult = '阴性'
  getImportantListData = [people1, people2, people3]

  // 展示 地图报点信息
  const people1Data = areaList.find(
    v => String(v.cardId) === String(people1.cardId)
  )
  people1Data.warnLevel = 4
  const people2Data = areaList.find(
    v => String(v.cardId) === String(people2.cardId)
  )
  importantListData = [people1Data, people2Data]
  const people3Data = areaList.find(
    v => String(v.cardId) === String(people3.cardId)
  )
  people3Data.warnLevel = 4
  gatherListData = [people3Data]

  // 展示密接信息
  const useCardIds = getImportantListData.map(v => v.cardId)
  closePersonListData = closeList.filter(v => {
    return useCardIds.indexOf(v.cardId) !== -1
  })
}
...
export function step12() {
  ...
  people2.testDate = '2020-12-23'
  people2.testResult = '阴性'

  people3.testDate = '2020-12-22'
  people3.testResult = '阳性'
  getImportantListData = [people1, people2, people3, people4, people5]
...
export function branch1(init) {
  const arr = [step1, step2, step3, step4, step5]
  arr.forEach((v, i) => {
    setTimeout(() => {
      v()
      init()
...
export function branch2(init) {
  // const arr = [step1, step2, step4, step5];
  // arr.forEach((v, i) => {
  //   setTimeout(() => {
  //     v();
  //     init();
