...
<script>
import { syndromeDictionary, syndromeHospital } from '@/api/real-api'
...
const typeCode = {
  1: { field: 'flight', label: '高危预警', otherParam: { warnLevel: 3 }},
  2: { field: 'ship', label: '一般性预警', otherParam: { warnLevel: 1 }},
  3: { field: 'school', label: '全部医院', otherParam: { warnLevel: 0 }}
}
    ...
    beforeIndexName: {
      ...
      render: {
        ...
        render(createElement, context) {
          return <span>{context.props.row.beforeIndexValue}</span>
        }
      }
    },
    indexName: {
      ...
      render: {
        ...
        props: { row: {}},
        render(createElement, context) {
          return <span>{context.props.row.indexValue}</span>
        }
      }
    },
    compareValue: '环比',
...
export default {
  ...
  methods: {
    testClick(data) {
      this.$emit('detail', { hospital: data.hospital })
    },
    initSearch() {
    ...
    fetchData() {
      ...
      api({
        ...
        if (res.data.length > 0) {
          ...
          fields.beforeIndexName.label = res.data[0].beforeIndexName
          fields.indexName.label = res.data[0].indexName
