<template>
  <div class="content">
    <i v-if="current !== 'city'" class="el-icon-back back" @click="back" />
    ...
    <hospital-modal-list
      v-else-if="current === 'school'"
      :props-param="modelData"
      @detail="toDetail"
    />
    <patient-hospital-modal-list
      v-else-if="current === 'student'"
      :props-param="studentData"
      @detail="toDetailModel"
    />
    <aggregation-test v-else-if="current === 'detail'" :data="detailData" />
  </div>
</template>

<script>
import HospitalModalList from './HospitalModalList'
import PatientHospitalModalList from './PatientHospitalModalList'
...
const singleObj = {
  fields: {
    ...
    beforeNum: '上周同期病例数',
    compareValue: '环比',
    normalRiskNum: '一般风险性医院数量（环比上涨30%-60%）',
    highRiskNum: '高风险性医院数量（环比上涨60%以上）',
    noRiskNum: '无风险性医院数量（环比上涨30%以下）',
    noTestNum: '未检测人数',
    positiveNum: '阳性人数',
    negativeNum: '阴性人数',
...
const step = ['city', 'school', 'student', 'detail'] // 定义层级

export default {
  components: {
    HospitalModalList,
    PatientHospitalModalList,
    AggregationTest
  },
  props: {
    propsParam: {
  ...
  methods: {
    back() {
      const index = step.findIndex(v => {
        return String(v) === String(this.current)
      })
      if (index > 0) {
        this.current = step[index - 1]
      }
    },
    testClick(data) {
      this.modelData = {
    ...
    toDetailModel(data) {
      ...
      Promise.all([
        ...
        getClosePersonList({ name, cardId, page: 1, limit: 100 })
      ]).then(res => {
        const getPersonnelInfoData = res[0].data
        const getClosePersonListData = res[1].data
        this.detailData = getPersonnelInfoData
        this.detailData.closePersonListData = getClosePersonListData
        this.current = 'detail'
      })
    },
    initSearch() {
