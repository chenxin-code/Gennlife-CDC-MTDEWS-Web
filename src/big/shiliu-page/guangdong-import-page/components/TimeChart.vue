    ...
    return {
      options: {
        ...
        xAxis: {
          type: 'category',
          splitNumber: 0,
          data: new Array(50).fill().map((v, i) => '2020/11/' + formatNumber(i + 1)),
          axisLabel: {
        ...
        legend: {
          ...
          data: [
            { name: '省外输入无症状感染', icon: 'roundRect' },
            { name: '省外输入确诊', icon: 'roundRect' }
          ]
        },
        series: [
          {
            data: new Array(50).fill().map((v, i) => random(100, 800)),
            type: 'bar',
            barWidth: 8,
            name: '省外输入确诊',
            stack: 'a'
