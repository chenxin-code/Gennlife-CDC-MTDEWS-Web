...
export default {
  ...
  data() {
    return {
      options: {
        ...
        legend: {
          data: ['跨境司机', '商务公务人员', '船员', '跨境学员', '全部豁免人员'],
          icon: 'rect',
          textStyle: {
  ...
  methods: {
    init() {
      ...
      this.$set(this.options.series[0], 'data', this.data.map((v) => { return v.driver }))
      this.$set(this.options.series[1], 'data', this.data.map((v) => { return v.business }))
      this.$set(this.options.series[2], 'data', this.data.map((v) => { return v.boatman }))
      this.$set(this.options.series[3], 'data', this.data.map((v) => { return v.student }))
      this.$set(this.options.series[4], 'data', this.data.map((v) => { return v.driver + v.business + v.boatman + v.student }))
