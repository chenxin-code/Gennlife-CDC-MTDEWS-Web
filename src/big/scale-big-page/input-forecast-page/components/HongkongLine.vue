<template>
  <v-chart :options="options" class="chart" />
</template>

<script>
import VChart from 'vue-echarts'
import 'echarts/lib/chart/line' // 自定义类型
import 'echarts/lib/component/tooltip'
import 'echarts/lib/component/dataZoom'
import { max } from 'lodash'

export default {
  components: {
    VChart
  },
  props: {
    data: Array,
    risk: Object,
    value: String
  },
  data() {
    return {
      options: {
        grid: {
          // x: 100,
          // y: 20,
          // x2: 20,
          // y2: 20
        },

        color: [
          {
            type: 'linear',
            x: 0,
            y: 0,
            x2: 0,
            y2: 1,
            colorStops: [
              {
                offset: 0,
                color: 'rgba(91, 241, 253, 1)' // 0% 处的颜色
              },
              {
                offset: 1,
                color: 'rgba(0, 153, 255, 1)' // 100% 处的颜色
              }
            ]
          }
        ],
        tooltip: {
          trigger: 'axis',
          axisPointer: {
            // 坐标轴指示器 快捷方式设置
            animation: false,
            type: 'cross'
          },
          formatter: (params, ticket, callback) => {
            if (this.value === '全部豁免人员') {
              if (params[0].data >= this.risk.r_xg_threshold) {
                return (params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data + '<br />' + this.risk.r_xg_warning_text)
              } else {
                return (
                  params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data
                )
              }
            } else if (this.value === '跨境司机') {
              if (params[0].data >= this.risk.r_kjsj_threshold) {
                return (params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data + '<br />' + this.risk.r_kjsj_warning_text)
              } else {
                return (
                  params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data
                )
              }
            } else if (this.value === '商务公务人员') {
              if (params[0].data >= this.risk.r_hmsw_threshold) {
                return (params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data + '<br />' + this.risk.r_hmsw_warning_text)
              } else {
                return (
                  params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data
                )
              }
            } else if (this.value === '船员') {
              if (params[0].data >= this.risk.r_hmcy_threshold) {
                return (params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data + '<br />' + this.risk.r_hmcy_warning_text)
              } else {
                return (
                  params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data
                )
              }
            } else if (this.value === '跨境学员') {
              if (params[0].data >= this.risk.r_hmxy_threshold) {
                return (params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data + '<br />' + this.risk.r_hmxy_warning_text)
              } else {
                return (
                  params[0].name +
                  "<br /><span style='display:inline-block;margin-right:5px;border-radius:2px;width:10px;height:5px;background-color:red;'></span>" +
                  '风险值：' + params[0].data
                )
              }
            }
          }
        },
        xAxis: {
          type: 'category',
          // nameRotate:30,
          splitNumber: 0,
          data: [],
          boundaryGap: false,
          axisLabel: {
            color: 'rgba(255, 255, 255, 0.5)',
            fontSize: 16
            // interval:0,
            // rotate:45
          },
          axisTick: {
            show: false
          },
          axisLine: {
            show: false
          }
        },
        yAxis: {
          max: '',
          type: 'value',
          axisLabel: {
            color: 'rgba(255, 255, 255, 0.5)',
            fontSize: 16
          },
          axisTick: {
            show: false
          },
          axisLine: {
            show: false
          },
          splitLine: {
            lineStyle: {
              color: 'rgba(151, 151, 151, 0.1)'
            }
          }
        },
        series: [
          {
            data: [],
            name: '风险值',
            type: 'line',
            itemStyle: {
              opacity: 0
            },
            markLine: {
              data: [
                { yAxis: 70, name: '报警阈值' }
              ],
              symbolSize: 0, // 标线两端的标记大小
              // label:{},
              lineStyle: {
                color: 'red'
              }
            },
            areaStyle: {
              normal: {
                color: {
                  type: 'linear',
                  x: 0,
                  y: 0,
                  x2: 0,
                  y2: 1,
                  colorStops: [
                    {
                      offset: 0,
                      color: 'rgba(91, 241, 253, 0.3)' // 0% 处的颜色
                    },
                    {
                      offset: 0.4,
                      color: 'rgba(91, 241, 253, 0.1)' // 0% 处的颜色
                    },
                    {
                      offset: 1,
                      color: 'rgba(0, 153, 255, 0.01)' // 100% 处的颜色
                    }
                  ]
                }
              }
            }
          }
        ],
        dataZoom: [
          {
            type: 'slider',
            show: true,
            // start: 0,
            // end: 40,
            fillerColor: '#191b36',
            handleIcon:
              'image://data:image/svg+xml;base64,'
          }
        ]
      }
    }
  },
  watch: {
    data() {
      this.init()
    },
    value() {
      this.init()
      // console.log(this.risk)
    }
  },
  mounted() {
    this.init()
  },
  methods: {
    init() {
      this.$set(this.options.xAxis, 'data', this.data.map((v) => { return v.dates }))
      if (this.value === '全部豁免人员') {
        this.$set(this.options.series[0], 'data', this.data.map((v) => { return v.risk_xg }))
        // console.log(max(this.data.map((v) => { return v.risk_xg }))*0.1)
        this.$set(this.options.yAxis, 'max', Math.round(max(this.data.map((v) => { return v.risk_xg })) * 1.1))
      } else if (this.value === '跨境司机') {
        this.$set(this.options.series[0], 'data', this.data.map((v) => { return v.risk_kjsj }))
        this.$set(this.options.yAxis, 'yAxis', Math.round(max(this.data.map((v) => { return v.risk_kjsj })) * 1.1))
      } else if (this.value === '商务公务人员') {
        this.$set(this.options.series[0], 'data', this.data.map((v) => { return v.risk_hmsw }))
        this.$set(this.options.yAxis, 'yAxis', Math.round(max(this.data.map((v) => { return v.risk_hmsw })) * 1.1))
      } else if (this.value === '船员') {
        this.$set(this.options.series[0], 'data', this.data.map((v) => { return v.risk_hmcy }))
        this.$set(this.options.yAxis, 'yAxis', Math.round(max(this.data.map((v) => { return v.risk_hmcy })) * 1.1))
      } else if (this.value === '跨境学员') {
        this.$set(this.options.series[0], 'data', this.data.map((v) => { return v.risk_hmxy }))
        this.$set(this.options.yAxis, 'yAxis', Math.round(max(this.data.map((v) => { return v.risk_hmxy })) * 1.1))
      }
    }
  }
}
</script>

<style lang="scss" scoped>
.chart {
  width: 100%;
  height: 75%;
}
</style>
